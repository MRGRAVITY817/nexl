...
pub const UNCLOSED_STRING: ErrorCode = ErrorCode { phase: ErrorPhase::Lexer, number: 1 };

/// Unrecognised or malformed escape sequence inside a string or character literal.
pub const INVALID_ESCAPE: ErrorCode = ErrorCode { phase: ErrorPhase::Lexer, number: 2 };

/// Character literal is malformed (empty, multi-char, invalid Unicode code point, etc.).
pub const INVALID_CHAR_LITERAL: ErrorCode = ErrorCode { phase: ErrorPhase::Lexer, number: 3 };

/// Integer or float literal suffix is unrecognised (e.g. `42x`).
pub const INVALID_NUMERIC_SUFFIX: ErrorCode = ErrorCode { phase: ErrorPhase::Lexer, number: 4 };
...
pub const UNMATCHED_DELIMITER: ErrorCode = ErrorCode { phase: ErrorPhase::Reader, number: 1 };

/// End-of-file was reached while a delimiter was still open.
pub const UNCLOSED_DELIMITER: ErrorCode = ErrorCode { phase: ErrorPhase::Reader, number: 2 };

/// A map literal has an odd number of forms (keys without values).
pub const ODD_MAP_FORMS: ErrorCode = ErrorCode { phase: ErrorPhase::Reader, number: 3 };
