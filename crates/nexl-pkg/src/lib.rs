...
/// The `:package` section in `project.nx`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Errors returned when parsing a `project.nx` manifest.
#[derive(Debug, Error)]
...
/// Parse a `project.nx` manifest (EDN format) into its schema representation.
